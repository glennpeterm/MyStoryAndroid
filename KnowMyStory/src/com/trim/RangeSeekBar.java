...
public class RangeSeekBar<T extends Number> extends ImageView {
    ...
    private final Bitmap thumbImageLeft = BitmapFactory.decodeResource(getResources(), R.drawable.trim_left_control);
    private final Bitmap thumbImageRight = BitmapFactory.decodeResource(getResources(), R.drawable.trim_right_control);
    private final Bitmap thumbPressedImage = BitmapFactory.decodeResource(getResources(),
                                                                          R.drawable.trim_left_control);
    private final Bitmap thumbDisabledImage = BitmapFactory.decodeResource(getResources(),
                                                                           R.drawable.trim_right_control);
    private final float thumbWidth = thumbImageLeft.getWidth();
    private final float thumbWidthRight = thumbImageRight.getWidth();
    ...
    TrimActivity trimActivity = new TrimActivity();
    /**
     * Default color of a {@link RangeSeekBar}, #FF33B5E5. This is also known as "Ice Cream Sandwich" blue.
     */
   // public static final int DEFAULT_COLOR = Color.argb(0xFF, 0x33, 0xB5, 0xE5);
    //public static final int DEFAULT_COLOR = null;
    ...
    public T getAbsoluteMinValue() {
    	System.out.println("absolute min value: "+absoluteMinValue);
    ...
    public T getSelectedMinValue() {
    	System.out.println("min value: "+normalizedToValue(normalizedMinValue));
            ...
            case MotionEvent.ACTION_MOVE:
                if (pressedThumb != null) {
                    ...
                    if (mIsDragging) {
                        ...
                        String max = String.valueOf(getSelectedMaxValue());
                        String min = String.valueOf(getSelectedMinValue());
                        
                        if(Thumb.MIN.equals(pressedThumb)){
                        	System.out.println("pressed min");
                        	trimActivity.setCurrentVideoFrame(min);
                        	trimActivity.setMinTimer(min);
                        }else if(Thumb.MAX.equals(pressedThumb)){
                        	System.out.println("pressed max");
                        	trimActivity.setCurrentVideoFrame(max);
                        	trimActivity.setMaxTimer(max);
                        }
                       // System.out.println("seek bar moved...");
                    } else {
    ...
    protected synchronized void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        int height = thumbImageLeft.getHeight() + PixelUtil.dpToPx(getContext(), HEIGHT_IN_DP);
    ...
    protected synchronized void onDraw(Canvas canvas) {
       ...
       // canvas.drawText(maxLabel, getWidth() - minMaxLabelSize, minMaxHeight, paint);
        ...
        mRect.right = mRect.right = getWidth() - 0;
        canvas.drawRect(mRect, paint);

        boolean selectedValuesAreDefault = (getSelectedMinValue().equals(getAbsoluteMinValue()) &&
                getSelectedMaxValue().equals(getAbsoluteMaxValue()));

        int DEFAULT_COLOR_ACTIVE = getResources().getColor(R.color.orange);
        int SEEK_COLOR_INACTIVE = getResources().getColor(R.color.black);
        int colorToUseForButtonsAndHighlightedLine = selectedValuesAreDefault ?
        		SEEK_COLOR_INACTIVE :    // default values
                	DEFAULT_COLOR_ACTIVE; //non default, filter is active
...
*/
        ...
        if (!selectedValuesAreDefault) {
            ...
            if (!mSingleThumb) {
            	...
            	canvas.drawRect(0,5,normalizedToScreen(normalizedMinValue),110,paint);
            	canvas.drawRect(normalizedToScreen(normalizedMaxValue),5,getWidth() - 0,110,paint);
            	paint.setColor(Color.TRANSPARENT);
            	//canvas.drawRect(normalizedToScreen(normalizedMinValue),30,normalizedToScreen(normalizedMaxValue),80,paint);
            	
                canvas.drawText(minText,
                                normalizedToScreen(normalizedMinValue) - minTextWidth * 0.5f,
                                mDistanceToTop + mTextSize,
                                paint);
            	/*mRect_view = new RectF(normalizedToScreen(normalizedMinValue),
            			normalizedToScreen(normalizedMinValue) - minTextWidth * 0.5f,
    ...
    private void drawThumb(float screenCoord, boolean pressed, Canvas canvas, boolean areSelectedValuesDefault) {
        ...
        if (areSelectedValuesDefault) {
        	...
        	System.out.println("drawThumb else ");
            buttonToDraw = pressed ? thumbImageLeft : thumbImageRight;
        }
    ...
    private void drawThumbMin(float screenCoord, boolean pressed, Canvas canvas, boolean areSelectedValuesDefault) {
    	System.out.println("pressed: "+pressed);
        Bitmap buttonToDraw;
        if (areSelectedValuesDefault) {
            buttonToDraw = thumbDisabledImage;
        	...
        	System.out.println("drawThumb else ");
            buttonToDraw = pressed ? thumbImageLeft : thumbImageLeft;
        }

        
        canvas.drawBitmap(buttonToDraw, screenCoord - thumbWidth,
                          0,
                          paint);
    ...
    private void drawThumbMax(float screenCoord, boolean pressed, Canvas canvas, boolean areSelectedValuesDefault) {
        ...
        if (areSelectedValuesDefault) {
        	...
        	System.out.println("drawThumb else ");
            buttonToDraw = pressed ? thumbImageRight : thumbImageRight;
        }

        canvas.drawBitmap(buttonToDraw, screenCoord,
                          0,
                          paint);
    ...
    private void setNormalizedMinValue(double value) {
        ...
        //System.out.println("min value: "+normalizedMinValue);
        invalidate();
    }
    ...
    private T normalizedToValue(double normalized) {
       ...
       // return (T) numberType.toNumber(Math.round(v * 100) / 100d);
        return (T) numberType.toNumber(Math.round(v*1000));
    }
